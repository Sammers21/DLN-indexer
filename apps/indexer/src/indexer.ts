...
export class Indexer {
  ...
  async startIndexing(): Promise<void> {
    this.checkpoint = await this.checkpointStore.getCheckpoint(
      this.programType,
    );
    this.running = true;
      ...
      logger.info(
        {
          kind: this.kind,
          from: {
            signature: this.checkpoint.from.signature.slice(0, 16) + "...",
            blockTime: formatTime(this.checkpoint.from.blockTime),
          },
          to: {
            signature: this.checkpoint.to.signature.slice(0, 16) + "...",
            blockTime: formatTime(this.checkpoint.to.blockTime),
          },
        },
    ...
    while (this.running) {
      try {
        ...
        if (
          signaturesUpwards.length < config.indexer.batchSize &&
          this.checkpoint
        ) {
          const signaturesBackwards = await this.solana.getSignaturesForAddress(
            this.programId,
            {
        ...
        logger.error(
          { err, kind: this.kind },
          "Error during indexing, retrying...",
        );
  ...
  private async getForwardSignatures(): Promise<ConfirmedSignatureInfo[]> {
    ...
    if (!this.checkpoint) {
      const batch = await this.solana.getSignaturesForAddress(this.programId, {
        limit,
      });
    ...
    while (true) {
      ...
      const checkpointIndex = batch.findIndex(
        (sig) => sig.signature === checkpointSig,
      );
      if (checkpointIndex >= 0) {
        if (checkpointIndex > 0)
          newestFirst.push(...batch.slice(0, checkpointIndex));
        break;
      }
  ...
  private async processTransaction(
    ...
    try {
      ...
      if (this.kind === "OrderCreated") {
        ...
        return await this.processOrderFulfilled(
          eventsList,
          sigInfo,
          txBlockTime,
        );
      ...
      logger.debug(
        { err, signature: sigInfo.signature },
  ...
  private async updateCheckpoint(
    ...
    if (now - this.lastCheckpointSaveTime >= 1000) {
      await this.checkpointStore.setCheckpoint(
        this.programType,
        this.checkpoint,
      );
