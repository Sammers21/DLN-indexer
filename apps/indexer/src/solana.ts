...
export class SolanaClient {
  ...
  constructor(rpcUrl?: string, rps?: number) {
    ...
    logger.info(
      { rpcUrl: this.rpcUrl, rps: requestsPerSecond },
      "Solana client initialized with bottleneck rate limiter",
    );
  }
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
          ...
          if (is429) {
            ...
            logger.warn(
              { attempt: attempt + 1, delay, method: name },
  ...
  async getSignaturesForAddress(
    ...
    const { result, timeMs } = await this.withRetry(
  ...
  private maybeLogMetrics(): void {
    ...
    > = {
      getSignaturesForAddress: this.buildMetricsSnapshot(
        "getSignaturesForAddress",
      ),
  ...
  private buildMetricsSnapshot(method: RpcMethod): {
    count: number;
    errorCount: number;
    avgMs: number;
  } {
