...
describe("Indexer", () => {
  ...
  describe("updateCheckpoint", () => {
    it("initializes checkpoint on first signature", async () => {
      ...
      await internal.updateCheckpoint(
        makeSigInfo("sig-first", 1000),
        "forward",
      );
    ...
    it("does not persist when within save interval", async () => {
      ...
      await internal.updateCheckpoint(
        makeSigInfo("sig-no-persist", 300),
        "forward",
      );
    ...
    it("uses current time as fallback when blockTime is null", async () => {
      ...
      const sigInfo = {
        ...
        memo: null,
        slot: 0,
      } as unknown as ConfirmedSignatureInfo;
