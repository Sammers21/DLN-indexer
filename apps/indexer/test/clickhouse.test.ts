...
describe("ClickHouse query shaping", () => {
    it("adds date filters when from/to are provided", async () => {
        ...
        await clickhouse.getDailyVolume({ eventType: "created", from: "2024-01-01", to: "2024-01-31" });
        expect(calls).to.have.length(1);
        expect(calls[0].query).to.include("WHERE event_type = {eventType:String} AND date >= {from:Date} AND date <= {to:Date}");
        expect(calls[0].params).to.deep.equal({ eventType: "created", from: "2024-01-01", to: "2024-01-31" });
    });
